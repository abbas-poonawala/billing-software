...
// ========== SIMPLE STOCK FETCH (fallback when batch fails) ==========
async function getSingleStoreStock(gsapi: any, item: string, shade: string): Promise<{ stock: number; rowIndex: number }> {
  try {
    const res = await gsapi.spreadsheets.values.get({
      spreadsheetId: STORE_SHEET_ID,
