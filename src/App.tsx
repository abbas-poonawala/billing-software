...
export default function App() {
  ...
  const [shortages, setShortages] = useState<ShortageInfo[] | null>(null);
  ...
  const saveBill = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (err.shortages && err.shortages.length > 0) {
  ...
  const loadBillForEdit = async (bill: RetrievedBill) => {
    ...
    if (bill.customerId) {
      ...
      if (existing) {
        ...
        const fallback = await searchCustomersByPhone(bill.customerPhone);
  ...
  // rendering – now with a fragment and proper shortage modal
  return (
    <>
      ...
      {/* ✅ Shortage Modal – correctly placed inside the fragment */}
      {shortages && (
        <div style={styles.overlay}>
          <div style={styles.modal}>
            <h3 style={{ margin: "0 0 12px", fontSize: 16, fontWeight: 800 }}>⚠️ Insufficient Stock</h3>
            <ul style={{ margin: "0 0 20px", paddingLeft: 20 }}>
              {shortages.map((s, i) => (
                <li key={i} style={{ marginBottom: 6, fontSize: 13 }}>
                  <strong>{s.item}</strong> ({s.shade}) – Requested: {s.requestedQty}, Available: {s.availableQty}
                </li>
              ))}
            </ul>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button onClick={() => setShortages(null)} style={styles.cancelBtn}>
                Close
              </button>
            </div>
          </div>
